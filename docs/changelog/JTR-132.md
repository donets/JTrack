...
- Added retry logic for per-location ticket number allocation collisions (`P2002` / `P2034`) in:
  - `TicketsService.create()`
  - sync-side ticket creation path in `SyncService`.
- Sync create retries switch to server-assigned ticket number when client-provided number conflicts.
- Added regression tests for retry behavior in both tickets and sync service suites.

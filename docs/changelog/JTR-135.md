...
- Added missing comment interactions in ticket detail:
  - Enter-to-submit for comment composer (`Enter`, `Shift+Enter` for newline),
  - edit-own-comment action with modal flow,
  - microphone button with browser SpeechRecognition fallback.
- Added comment update capability in offline repository (`updateComment`) with outbox `ticketComments:update`.
- Added backend endpoint `PATCH /comments/{id}` for own-comment edits and ownership validation.

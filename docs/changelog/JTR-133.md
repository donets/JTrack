...
- Removed `status` from generic `UpdateTicketInput`/`updateTicketSchema` to eliminate bypass through `PATCH /tickets/{id}`.
- `PATCH /tickets/{id}` now updates non-status fields only; status changes are accepted exclusively via `PATCH /tickets/{id}/status`.
- Added regression coverage in `tickets.service.spec.ts` to ensure status is ignored in generic update payload.

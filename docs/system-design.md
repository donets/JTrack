...
- Validation is schema-based (Zod) and fails fast at controller boundary via `ZodValidationPipe`; pipe also normalizes JSON-string payloads (including double-serialized payloads) before schema validation to tolerate legacy clients.

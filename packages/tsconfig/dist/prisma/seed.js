...
async function syncRolesAndPrivileges() {
    for (const role of shared_1.roleKeys) {
    ...
    const privilegeSet = new Set(Object.values(shared_1.rolePrivileges).flat());
    ...
    for (const role of shared_1.roleKeys) {
        ...
        await prisma.rolePrivilege.createMany({
            data: shared_1.rolePrivileges[role].map((privilegeKey) => ({
                roleKey: role,
                privilegeKey
            })),
            skipDuplicates: true
...
async function seedDemoData() {
    const adminPasswordHash = await (0, bcryptjs_1.hash)('password123', 12);
    ...
    const ownerPasswordHash = await (0, bcryptjs_1.hash)('password123', 12);
    ...
    await prisma.ticket.upsert({
        ...
        update: {
            ...
            status: client_1.TicketStatus.Scheduled,
